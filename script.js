...
function clearUserCache() {
    ...
    const chunirecInput = document.getElementById('token-input');
...
/**
 * トークン入力欄の表示/非表示切り替え（引数で汎用化）
 * @param {string} inputId - 対象のinput要素のID（デフォルト: token-input）
 * @param {string} btnId - 対象のbutton要素のID（デフォルト: toggle-token）
 */
function toggleTokenVisibility(inputId = "token-input", btnId = "toggle-token") {
    const input = document.getElementById(inputId);
    const btn = document.getElementById(btnId);

    if (!input || !btn) {
        console.error("要素が見つかりません。IDが正しいか確認してください:", inputId, btnId);
        return;
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!isBothTokensSet) {
        console.warn("トークン未登録または不足のためログイン画面を表示します。");
 ...
 * 💡 スコア読み込み・同期
 */
async function loadScores(isRefresh = false) {
    const supportInput = document.getElementById("support-token-input");
    ...
    try {
        const response = await fetch(GAS_URL, {
            ...
            body: JSON.stringify({
                mode: "checker",
                chuniSupportToken: supportToken,
                chunirec_token: chunirecToken
            })
        });
...
/**
 * 画面にスコアを表示する（難易度セル：生定数表示 / レートセル：生値表示）
 */
function displayScores(data) {
    ...
    try {
        ...
        // 💡 右列用：生のRating値/傾向値を取得する共通関数（補正なし）
        const getRawRatingVal = (item, trend) => {
            if (!item || !trend) return 0;
            if (item.calcVal !== undefined && item.calcVal !== null) {
                return parseFloat(item.calcVal);
            }

            if (trend === 'POWER') return parseFloat(item.tairyoku ?? item.rawTairyoku ?? 0);
            if (trend === 'NOTES') return parseFloat(item.kenban ?? item.rawKenban ?? 0);
            if (trend === 'CHUNI') return parseFloat(item.chuni ?? item.rawChuni ?? 0);
            if (trend === 'TRICKY') return parseFloat(item.kuse ?? item.rawKuse ?? 0);
        ...
        // 💡 傾向ごとのハイライト枠数の判定（ABILITY_LIMITS を参照）
        const limitsMap = (typeof ABILITY_LIMITS !== 'undefined') ? ABILITY_LIMITS : {
            tairyoku: 30,
            kenban: 50,
            chuni: 30,
            kuse: 16
        ...
        // 💡 傾向選択時の枠対象（Top N）の算出（生の算出値で判定）
        const targetSet = new Set();
        if (selectedTrend) {
            const limit = getTrendLimit(selectedTrend);
            const sourceData = (typeof myCurrentRecords !== "undefined" && Array.isArray(myCurrentRecords) && myCurrentRecords.length > 0)
                ? myCurrentRecords
                : data;

            [...sourceData]
                .sort((a, b) => getRawRatingVal(b, selectedTrend) - getRawRatingVal(a, selectedTrend))
                .slice(0, limit)
                .forEach(item => {
                    if (item && item.title && item.diff) {
                        targetSet.add(`${item.title}_${item.diff}`);
                    }
                });
        }
        ...
        limitedData.forEach((item, index) => {
            ...
            if (selectedTrend) {
                // 生の傾向値を取得して表示
                const ratingVal = getRawRatingVal(item, selectedTrend);
...
function calcPlayerAbilityAverages(targetData) {
    ...
    keys.forEach(k => {
        const count = ABILITY_LIMITS[k.modKey] || 30;
        const songs = getTopAbilitySongs(targetData, k.modKey, k.rawKey, count);
...
/**
 * 💡 ログアウト処理
 * キャッシュおよび保存されたトークン類を削除して画面をリロードする
 */
function logout() {
    if (confirm("ログアウトしますか？（保存されたスコアやトークンが消去されます）")) {
        // localStorageのデータを削除（chunisupportトークンも追加）
        localStorage.removeItem('chunisupport_token');
        localStorage.removeItem('chunirec_token');
        localStorage.removeItem('chunirec_scores');
        localStorage.removeItem('chunirec_player_name');
        ...
        // 他にも独自で保持しているキャッシュ消去関数があれば実行
        if (typeof clearUserCache === 'function') {
            clearUserCache();
        }

        // ページをリロードして初期画面（トークン入力画面）に戻す
